...
def random_sampling(pred_probs: torch.Tensor, budget: int = 1000) -> List[int]:
    ...
    indices = torch.randperm(len(pred_probs))[:budget].tolist()
    # ================================
    # FILL ME OUT
...
def uncertainty_sampling(pred_probs: torch.Tensor, budget: int = 1000) -> List[int]:
    ...
    # HINT: please ensure indices is a list of integers
    # ================================
    # Calculate the confidence as the max predicted probability for each example
    confidences = torch.max(pred_probs, dim=1).values

    # Sort indices by confidence in ascending order (least confident first)
    sorted_indices = torch.argsort(confidences)

    # Select the first `budget` indices
...
def margin_sampling(pred_probs: torch.Tensor, budget: int = 1000) -> List[int]:
    ...
    sorted_probs, _ = torch.sort(pred_probs, dim=1, descending=True)
...
def entropy_sampling(pred_probs: torch.Tensor, budget: int = 1000) -> List[int]:
    ...
    # Sort indices by entropy in descending order (highest entropy first)
