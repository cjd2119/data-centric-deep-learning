...
def get_ks_score(tr_probs, te_probs):
    ...
    tr_prob = tr_probs.numpy()
    te_prob = te_probs.numpy()
    ks_test = ks_2samp(tr_prob, te_prob)
    score = ks_test.pvalue
...
def get_hist_score(tr_probs, te_probs, bins=10):
    ...
    te_heights, _ = np.histogram(te_probs, density=True, bins=bin_edges)
...
class MonitoringSystem:
    ...
    def monitor(self, te_vocab, te_probs):
        tr_probs, te_probs = self.calibrate(
            self.tr_probs, self.tr_labels, te_probs)
